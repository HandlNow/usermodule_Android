...
import com.handlUser.app.databinding.FragmentGraphBinding
...
class GraphFragment : BaseFragment(), ClickHandler, BaseAdapter.OnPageEndListener {
    ...
    private fun hitGetRatingAPI() {
        if (!isSingle) {
            isSingle = true
            val call = api!!.apiGetRating(baseActivity!!.getProfileData()!!.id?:0, pageCount)
            restFullClient!!.sendRequest(call, this)
        }
    ...
    private fun getAreaCount(): ArrayList<String> {
        ...
        for (i in 0 until earningList.size) {
            label.add(earningList[i].createdOn)
        }
        return label
    }

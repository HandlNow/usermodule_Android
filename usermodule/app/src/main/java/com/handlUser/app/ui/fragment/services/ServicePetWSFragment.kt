...
import com.handlUser.app.databinding.FragmentServicePetWsBinding
...
import com.handlUser.app.ui.adapter.service.PetServiceAdapter
...
class ServicePetWSFragment : BaseFragment(), ClickHandler, BaseAdapter.OnItemClickListener, BaseAdapter.OnPageEndListener {

    private var binding: FragmentServicePetWsBinding? = null
    private var alertDialog: AlertDialog? = null
    private var data: SubCategory? = null
    private var list: ArrayList<ServiceData> = arrayListOf()
    private var isSingle = false
    private var pageCount = 0
    private var adapter: PetServiceAdapter? = null

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        if (binding == null)
            binding = DataBindingUtil.inflate(
                    inflater, R.layout.fragment_service_pet_ws, container, false)
        return binding!!.root
    }
    ...
    private fun initUI() {
        ...
        baseActivity!!.setToolbar(getString(R.string.pet_ws))
        resetList()
        hitGetDataAPI()
    }
    ...
    private fun setAdapter() {
        if (adapter == null) {
            adapter = PetServiceAdapter(baseActivity!!, list[0].parent)
    ...
    private fun getJsonArray(): JSONArray {
        ...
        for (data in list[0].parent) {
            if (data.value > 0) {
                ...
                jsonObject.put("is_parent_quantity", "")
                jsonObject.put("kid_id", "")
                jsonObject.put("children_id", "")
                jsonObject.put("is_children_check", "")
                jsonObject.put("is_children_quantity", "")

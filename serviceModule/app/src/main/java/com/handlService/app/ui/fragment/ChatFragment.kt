...
import com.handlService.app.databinding.FragmentChatBinding
import com.handlService.app.model.ChatData
import com.handlService.app.ui.activity.BaseActivity
import com.handlService.app.ui.adapter.ChatAdapter
        ...
        try {
                ...
                if (responseUrl.contains(Const.API_SEND_MESSAGE)) {
                    ...
                    if (responseCode == Const.STATUS_OK) {
                        ...
                        if (jsonObject.has(Const.LIST)) {
                            if (jsonObject.getJSONArray(Const.LIST).length() > 0) {
                                for (k in 0 until jsonObject.getJSONArray(Const.LIST).length()) {
                                    val data = Gson().fromJson(jsonObject.getJSONArray(Const.LIST).getJSONObject(k).toString(), ChatData::class.java)

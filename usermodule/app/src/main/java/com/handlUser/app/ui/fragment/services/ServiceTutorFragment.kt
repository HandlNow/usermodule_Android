...
import com.handlUser.app.databinding.FragmentServicePrimaryTutorBinding
...
import com.handlUser.app.ui.adapter.service.NannyAdapter
...
class ServiceTutorFragment : BaseFragment(), ClickHandler, BaseAdapter.OnItemClickListener, BaseAdapter.OnPageEndListener {

    private var binding: FragmentServicePrimaryTutorBinding? = null
    private var alertDialog: AlertDialog? = null
    private var data: SubCategory? = null
    private var list: ArrayList<ServiceData> = arrayListOf()
    private var isSingle = false
    private var pageCount = 0
    private var adapter: NannyAdapter? = null

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        if (binding == null)
            binding = DataBindingUtil.inflate(
                    inflater, R.layout.fragment_service_primary_tutor, container, false)
        return binding!!.root
    }
    ...
    private fun initUI() {
        ...
        baseActivity!!.setToolbar(getString(R.string.primary_tutor))
        hitGetDataAPI()
    }
    ...
    private fun hitAddKidApi() {
        val count = list[0].parentKid.size + 1
        val param = Api3Params()
        param.put("Kid[title]", "Kid $count")
        param.put("Kid[type_id]", data!!.typeId)
        val call = api!!.apiAddKid(param.getServerHashMap())
        restFullClient!!.sendRequest(call, this)
    }
    ...
    override fun onItemClick(vararg itemData: Any) {
        when (itemData[0] as Int) {
            ...
            Const.TWO -> {
                ...
                list[0].parentKid[pos].value = text.toInt()
    ...
    override fun onSyncSuccess(responseCode: Int, responseMessage: String, responseUrl: String, response: String?) {
        ...
        try {
            ...
            } else if (responseUrl.contains(Const.API_PROVIDER_ADD_KID)) {
                if (responseCode == Const.STATUS_OK) {
                    val data = Gson().fromJson<ParentData>(jsonobject.getJSONObject("detail").toString(), ParentData::class.java)
                    list[0].parentKid.add(data)
    ...
    private fun setAdapter() {
        if (adapter == null) {
            adapter = NannyAdapter(baseActivity!!, list[0].parentKid, this)

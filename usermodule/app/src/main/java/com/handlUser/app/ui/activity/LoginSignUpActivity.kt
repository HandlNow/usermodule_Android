...
import com.handlUser.app.databinding.ActivityLoginBinding
import com.handlUser.app.ui.extensions.replaceFragment
import com.handlUser.app.ui.fragment.*
import com.handlUser.app.ui.fragment.login.*
...
class LoginSignUpActivity : BaseActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (getProfileData() != null) {
            if (getProfileData()!!.isUserAddress) {
                gotoPaymentFagment()
            } else {
                gotoAddressFagment()
            }
    ...
    private fun gotoPaymentFagment() {
        supportFragmentManager.popBackStack(null, FragmentManager.POP_BACK_STACK_INCLUSIVE)
        replaceFragment(PaymentMethodListFragment())

    }

    private fun gotoAddressFagment() {
        supportFragmentManager.popBackStack(null, FragmentManager.POP_BACK_STACK_INCLUSIVE)
        replaceFragment(RegisterHouseFragment())
    ...
    override fun onBackPressed() {
        ...
        if (fragment is LanguageSelectionFragment
                || fragment is SelectionFragment ||
                fragment is AddUserDetailFragment ||
                fragment is RegisterHouseFragment ||
                fragment is PaymentMethodListFragment) {
            backAction()

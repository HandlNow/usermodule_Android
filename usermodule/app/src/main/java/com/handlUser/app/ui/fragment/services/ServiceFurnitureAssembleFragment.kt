...
import com.handlUser.app.databinding.FragmentServiceFunitureBinding
...
import com.handlUser.app.ui.adapter.service.FurnitureAssembleAdapter
...
class ServiceFurnitureAssembleFragment : BaseFragment(), ClickHandler, BaseAdapter.OnItemClickListener, BaseAdapter.OnPageEndListener {

    private var binding: FragmentServiceFunitureBinding? = null
    ...
    private var adapter: FurnitureAssembleAdapter? = null

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        if (binding == null)
            binding = DataBindingUtil.inflate(
                    inflater, R.layout.fragment_service_funiture, container, false)
        return binding!!.root
    }
    ...
    private fun initUI() {
        ...
        baseActivity!!.setToolbar(getString(R.string.furniture_assemble))
        hitGetDataAPI()
    }
    ...
    private fun setAdapter() {
        if (adapter == null) {
            adapter = FurnitureAssembleAdapter(baseActivity!!, list[0].parent)

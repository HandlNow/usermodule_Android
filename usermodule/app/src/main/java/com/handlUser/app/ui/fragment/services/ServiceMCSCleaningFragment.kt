...
import com.handlUser.app.databinding.FragmentServiceMatresSofaCleaningBinding
...
import com.handlUser.app.ui.adapter.service.MattressCleaningAdapter
...
class ServiceMCSCleaningFragment : BaseFragment(), ClickHandler, BaseAdapter.OnItemClickListener, BaseAdapter.OnPageEndListener {

    private var binding: FragmentServiceMatresSofaCleaningBinding? = null
    private var alertDialog: AlertDialog? = null
    private var data: SubCategory? = null
    private var list: ArrayList<ServiceData> = arrayListOf()
    private var isSingle = false
    private var pageCount = 0
    private var adapter: MattressCleaningAdapter? = null


    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        if (binding == null)
            binding = DataBindingUtil.inflate(
                    inflater, R.layout.fragment_service_matres_sofa_cleaning, container, false)
        return binding!!.root
    }
    ...
    private fun initUI() {
        ...
        baseActivity!!.setToolbar(getString(R.string.mcs_cleaning))
        hitGetDataAPI()
    }
    ...
    override fun onItemClick(vararg itemData: Any) {
        when (itemData[0] as Int) {
            Const.TWO -> {
                ...
                list[0].parent[checkPosition].children[pos].value = text.toInt()
    ...
    private fun setAdapter() {
        if (adapter == null) {
            adapter = MattressCleaningAdapter(baseActivity!!, list[0].parent, this)
    ...
    var checkPosition = 0
    fun showDialog(position: Int, checkPosition: Int) {
        pos = position

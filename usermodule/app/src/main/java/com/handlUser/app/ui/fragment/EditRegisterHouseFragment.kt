...
class EditRegisterHouseFragment : BaseFragment(), AddressDecoder.AddressListener, BaseActivity.PermCallback, OnMapReadyCallback, GoogleMap.OnCameraIdleListener, GoogleMap.OnMapLoadedCallback, ClickHandler, BaseAdapter.OnItemClickListener {
    ...
    private fun isValidate(): Boolean {
        when {
            binding!!.addressET.isBlank()
                    ...
                    || binding!!.cityET.isBlank() -> baseActivity!!.showToastOne(baseActivity!!.getString(R.string.enter_complemte_data))
            else -> {
                return true
            }
        }
        ...
        try {
            ...
            if (responseUrl.contains(Const.API_ADD_ADDRESS)) {
                if (responseCode == Const.STATUS_OK) {
                    showToastOne(getString(R.string.adrress_added))
                    val detail = jsonObjects.getJSONObject("detail")
                    val data = Gson().fromJson<AddressData>(detail.toString(), AddressData::class.java)
                ...
                if (responseCode == Const.STATUS_OK) {
                    ...
                    for (i in 0 until jsonArray.length()) {
                        val data = Gson().fromJson<AddressData>(jsonArray.toString(), AddressData::class.java)
                        userAddressList.add(data)
                    }
                    setAdapter()
...
//                        showToastOne(getString(R.string.not_able_to_get_latlong))
    ...
    override fun onPlacesAddressFound(addressData: AddressData?) {
        baseActivity?.runOnUiThread {
            if (data != null) {
                ...
                binding!!.doneTV.text = baseActivity!!.getString(R.string.update_homes)
            } else {
                this.addressData = addressData
            }
            if (this.addressData != null) {
                if (this.addressData!!.isZoomMap) {
                    getData = false
                    googleMap!!.moveCamera(CameraUpdateFactory.newLatLngZoom(LatLng(this.addressData!!.latitude?.toDouble()
                            ?: 0.0, this.addressData!!.longitude?.toDouble() ?: 0.0), 15f))
                }
                binding!!.addressET.setText(this.addressData!!.address.toString())
                binding!!.codeET.setText(this.addressData!!.zipcode.toString())
                binding!!.cityET.setText(this.addressData!!.city.toString())
    ...
    private fun getAddress() {
        if (!Places.isInitialized()) {
            Places.initialize(baseActivity!!.applicationContext, baseActivity!!.getString(R.string.google_api))
        }

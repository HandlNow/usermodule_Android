...
import com.handlUser.app.databinding.FragmentServiceAcCleaningBinding
...
import com.handlUser.app.ui.adapter.service.AcCleaningAdapter
...
class ServiceAcCleaningFragment : BaseFragment(), ClickHandler, BaseAdapter.OnItemClickListener, BaseAdapter.OnPageEndListener {

    private var binding: FragmentServiceAcCleaningBinding? = null
    private var alertDialog: AlertDialog? = null
    private var data: SubCategory? = null
    private var list: ArrayList<ServiceData> = arrayListOf()
    private var isSingle = false
    private var pageCount = 0
    private var adapter: AcCleaningAdapter? = null

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        if (binding == null)
            binding = DataBindingUtil.inflate(
                    inflater, R.layout.fragment_service_ac_cleaning, container, false)
        return binding!!.root
    }
    ...
    private fun initUI() {
        ...
        baseActivity!!.setToolbar(getString(R.string.ac_cleaning))
        hitGetDataAPI()
    }
    ...
    private fun setAdapter() {
        if (adapter == null) {
            adapter = AcCleaningAdapter(baseActivity!!, list[0].parent)
